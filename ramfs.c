...
int ropen(const char *pathname, int flags) {
	...
	for (a = 0; a < lenth; a++)
	{
		...
		if (pathname[a] != 47)
		{
			...
			{
				FirstLetter = a + 1;
				for (b = 0; b < find->biggest; b++)
				{
					if (find->dirents[b] == NULL) continue;
					if (strcmp(find->dirents[b]->name, name_of_find) == 0)//why 断开
					{
						find = find->dirents[b];//why 断开
						strcpy_s(name_of_find, 32, clear);//如果找到了子文件，则name_of_find为clear///////////!!!!!
						break;
					}
					else if (b == find->biggest - 1) return -1;
				}
				if (name_of_find[0] != 0)
				{
	...
	if (find->type == 0)
	{
		if (name_of_find[0] != 0)//need to creat
		{
			...
			file->type = 0;
			file->name = name_of_find;
			int last = find->biggest;
			find->biggest++;
			find->size++;
			find->dirents[last] = file;
			flags = flags - 64;
		}
...
ssize_t rwrite(int fd, const void *buf, size_t count) {
	...
	unsigned int b = strlen(FD[fd].f->content);
	if (offset + count > b)
	{
...
int rmkdir(const char *pathname) {
	...
	dir->type = 1;
	dir->name = name_of_find;
	find->dirents[big] = dir;
	return 0;
}
